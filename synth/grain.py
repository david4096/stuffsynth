...
class PCA_Synthesizer:
    def __init__(self, faiss_db_path, metadata_path, sample_rate=44100, block_size=1024, tween_steps=10, refresh_interval=0.1, max_voices=16):
        ...
        self.active_voices = []  # List to track active voices
    ...
    def stream_callback(self, indata, outdata, frames, time, status):
        ...
        # Log the current synth states
        self.log_synth_state()
    ...
    def log_synth_state(self):
        """Log the state of active voices."""
        if len(self.active_voices) > self.max_voices:
            self.active_voices = self.active_voices[:self.max_voices]

        for i, voice in enumerate(self.active_voices):
            logging.info(f"Voice {i+1}: PCA Vector: {voice['pca_vector']}, Frequency: {voice['frequency']}, Amplitude: {voice['amplitude']}, Time Stretch: {voice['time_stretch']}")

    def add_voice(self, pca_vector):
        """Add a new voice to the active voices."""
        if len(self.active_voices) >= self.max_voices:
            # Remove the oldest voice if max voices reached
            self.active_voices.pop(0)
        
        frequency = 440 * (1 + pca_vector[0])  # Modulate pitch
        amplitude = 0.5 * (1 + pca_vector[1])  # Modulate amplitude
        time_stretch = 1 + pca_vector[2]       # Modulate time stretch
